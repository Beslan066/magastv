                                    ...
                                    @if(isset($news->image_author))
                                        <div class="flex-row">
                                            ...
                                            @if(isset($news->image_description))
                                                <span
                                                    class="single-news__image_name">| {{$news->image_description}}</span>
                                            @endif
                                        </div>
                                    @endif
                                </div>
                                        ...
                                        @if($popularItems)
                                            @foreach($popularItems as $news)
                                                <li class="popular-sidebar__item">
                                                    <a href="{{route('home.news.single', $news->slug)}}"
                                                ...
                                                <div class="news-item__media">
                                                    @if(file_exists(public_path('storage/public/' . $item->media)))
                                                        <img src="{{asset('storage/public/' . $item->media)}}"
                                                             alt="{{$item->title}}">
                                                    @else
                                                        <img src="{{asset('assets/default-image.jpg')}}"
            ...
            <div class="container">
                <div class="single-news-content__inner">
                    <div class="single-news__top">
                        ...
                        <div class="single-news__info">
                            <time>{{$news->formatted_published_at}}</time>
                            @if(auth()->user() && auth()->user()->role->name !== 'Обычный пользователь')
                                <div class="flex-row">
                                    <a href="{{route('tidings.edit', $news->id)}}">Изменить</a>
                                    <form action="{{route('tidings.destroy', $news->id)}}" method="post">
                                        @csrf
                                        @method('delete')
                                        <button type="submit"
                                                onclick="return confirm('Вы точно хотите удалить эту новость?');">
                                    ...
                                    <ul class="list-reset popular-sidebar__list">
                                        @if($popularItems)
                                            @foreach($popularItems as $popularItem)
                                                <li class="popular-sidebar__item">
                                                    <a href="{{route('home.news.single', $popularItem->slug)}}"
                                                       class="popular-sidebar__item_text">
                                                        {{$popularItem->title}}
                                                    </a>
                                                    <div class="popular-sidebar__item_info">
                                                        <time datetime="{{$popularItem->formatted_published_at}}"
                                                              class="popular-sidebar__item_time">
                                                            {{$popularItem->formatted_published_at}}
                                                        </time>
                                                    </div>
                                                </li>
                                            @endforeach
