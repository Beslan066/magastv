...
@section('content')
    ...
    <div class="card">
        ...
        <form action="{{route('radio-broadcast.update', $radioBroadcast->id)}}" method="post" enctype="multipart/form-data">
            @csrf
            @method('patch')
            <div class="row mb-6 gy-6">
                <div class="col-xl">
                    <div class="card">
                        <div class="card-body">
                            <div class="mb-4">
                                <input type="text" class="form-control" placeholder="Заголовок" name="title" value="{{$radioBroadcast->title}}">
                            </div>
                            ...
                            <div class="mb-4">
                                <input type="text" class="form-control"  placeholder="URL (оставьте пустым для автоматического сгенерирования)" name="slug" value="{{$radioBroadcast->slug}}">
                            </div>

                            <div class="mb-4">
                                <div class="input-group">
                                    <textarea class="form-control" placeholder="Лид новости" name="lead">{{$radioBroadcast->lead}}</textarea>
                                </div>
                            </div>
                ...
                <div class="col-xl">
                    <div class="card">
                        <div class="card-body">
                            <div class="mb-4">
                                <div class="input-group">
                                    <input type="file" class="form-control" id="inputGroupFile02" name="image" accept="image/*" @if($radioBroadcast->image)
                                        data-default-file="{{ asset('storage/public/' . $radioBroadcast->image_main) }}"
                                           @endif>
                                ...
                                @if (isset($radioBroadcast->image))
                                <div class="old-image-wrapper mt-3 position-relative">
                                    <h4 class="text-secondary">Текущее изображение</h4>
                                    <img src="{{asset('storage/public/' . $radioBroadcast->image)}}" class="img-thumbnail old-image-preview" style="max-height: 200px">
                                </div>
                                @endif
                            ...
                            @if (isset($radioBroadcast->audio))
                                <div class="old-image-wrapper mt-3 position-relative">
                                    <h4 class="text-secondary">Текущее аудио</h4>
                                    <audio controls src="{{asset('storage/public/' . $radioBroadcast->audio)}}"></audio>
                                    <a href="{{asset('storage/public/' . $radioBroadcast->audio)}}" target="_blank">Скачать</a>
                                </div>
                            @endif
                            ...
                            <div class="form-group w-50 mb-4">
                                <div class="form-group w-50">
                                   ...
                                   padding: 5px !important; "
                                           value="{{ $radioBroadcast->published_at ? date('Y-m-d\TH:i', strtotime($radioBroadcast->published_at)) : '' }}"                            >
                                </div>
                            </div>
                            ...
                            <div class="form-check form-switch mb-2">
                                <input class="form-check-input" type="checkbox" id="flexSwitchCheckChecked" name="status" @if($radioBroadcast->status == 1)
                                    checked
                                    @endif>
