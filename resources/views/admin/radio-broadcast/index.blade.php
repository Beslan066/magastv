...
@section('content')
    ...
    <div class="card">
        <div class="d-flex align-items-center justify-content-between mb-2">
            <h5 class="card-header">Выпуски радио</h5>
            ...
            <div class="d-flex align-items-center">
                ...
                <a href="{{ route('radio-broadcast.create') }}" type="button" class="btn btn-primary waves-effect waves-light ms-2">Добавить</a>
            </div>
        </div>

        <!-- Фильтры и сортировка -->
        <div class="card-body border-bottom">
            <form method="GET" action="{{ route('radio-broadcast.index') }}" class="row g-3">
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    <label class="form-label">Радио-передача</label>
                    <select name="radio_transfer" class="form-select">
                        <option value="">Все передачи</option>
                        @foreach($radioTransfers as $transfer)
                            <option value="{{ $transfer->id }}" {{ request('radio_transfer') == $transfer->id ? 'selected' : '' }}>
                                {{ $transfer->title }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3">
                    ...
                    <select name="sort" class="form-select">
                        ...
                        <option value="published_desc" {{ request('sort') == 'published_desc' ? 'selected' : '' }}>По дате публикации (убыв.)</option>
                        <option value="published_asc" {{ request('sort') == 'published_asc' ? 'selected' : '' }}>По дате публикации (возр.)</option>
                ...
                <div class="col-12 d-flex justify-content-end">
                    <div class="btn-group">
                        ...
                        <a href="{{ route('radio-broadcast.index') }}" class="btn btn-outline-secondary">Сбросить</a>
                    </div>
                </div>
        ...
        <div class="table-responsive text-nowrap">
            <table class="table">
                ...
                @if(isset($radioBroadcast) && $radioBroadcast->count() > 0)
                    @foreach($radioBroadcast as $item)
                        <tr>
                            ...
                            <td>
                                @if($item->audio)
                                    <span class="badge bg-label-success"><i class="bx bx-volume-full"></i></span>
                                @else
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" href="{{ route('radio-broadcast.edit', $item->id) }}">
                                            <i class="icon-base bx bx-edit-alt me-1"></i>Изменить
                                        </a>
                                        <form action="{{ route('radio-broadcast.destroy', $item->id) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')
                ...
                @else
                    <tr>
                        <td colspan="8" class="text-center py-4">
                            <div class="text-muted">Выпуски радио не найдены</div>
                            @if(request()->anyFilled(['search', 'author', 'radio_transfer', 'status']))
                                <a href="{{ route('radio-broadcast.index') }}" class="btn btn-sm btn-outline-primary mt-2">
                                    Сбросить фильтры
                                </a>
                            @endif
                        </td>
