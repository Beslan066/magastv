...
@section('content')
    ...
    <div class="card">
        ...
        <form action="{{ route('radio-programs.update', $radioProgram->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                        ...
                        <div class="card-body">
                            <div class="mb-4">
                                <input type="text" class="form-control" placeholder="Название программы"
                                       name="title" value="{{ old('title', $radioProgram->title) }}" required>
                            </div>

                            <div class="input-group mb-4">
                                <select class="form-select" id="inputGroupSelect02" name="category_id">
                                    @if(isset($radioProgram->category_id))
                                        <option value="{{$radioProgram->category_id}}">{{$radioProgram->category->title}}</option>
                                    @else
