...
@section('content')
    ...
    <div class="card">
        ...
        <form action="{{route('radio-news.update', $radioNews->id)}}" method="post" enctype="multipart/form-data">
            @csrf
            @method('patch')
            <div class="row mb-6 gy-6">
                <div class="col-xl">
                    <div class="card">
                        <div class="card-body">
                            <div class="mb-4">
                                <input type="text" class="form-control" placeholder="Заголовок" name="title" value="{{$radioNews->title}}">
                            </div>
                            ...
                            <div class="mb-4">
                                <input type="text" class="form-control"  placeholder="URL (оставьте пустым для автоматического сгенерирования)" name="slug" value="{{$radioNews->slug}}">
                            </div>

                            <div class="mb-4">
                                <div class="input-group">
                                    <textarea class="form-control" placeholder="Лид новости" name="lead">{{$radioNews->lead}}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="mb-4">
                                <div class="input-group">
                                    <textarea class="form-control" placeholder="В эфире" name="in_air">{{$radioNews->in_air}}</textarea>
                                </div>
                            </div>
                ...
                <div class="col-xl">
                    <div class="card">
                        <div class="card-body">
                            <div class="mb-4">
                                <div class="input-group">
                                    <input type="file" class="form-control" id="inputGroupFile02" name="image" accept="image/*" @if($radioNews->image)
                                        data-default-file="{{ asset('storage/public/' . $radioNews->image_main) }}"
                                           @endif>
                                ...
                                @if (isset($radioNews->image))
                                <div class="old-image-wrapper mt-3 position-relative">
                                    <h4 class="text-secondary">Текущее изображение</h4>
                                    <img src="{{asset('storage/public/' . $radioNews->image)}}" class="img-thumbnail old-image-preview" style="max-height: 200px">
                                </div>
                                @endif
                            ...
                            <div class="form-group w-50 mb-4">
                                <div class="form-group w-50">
                                   ...
                                   padding: 5px !important; "
                                           value="{{ $radioNews->published_at ? date('Y-m-d\TH:i', strtotime($radioNews->published_at)) : '' }}"                            >
                                </div>
                            </div>
                            ...
                            <div class="form-check form-switch mb-2">
                                <input class="form-check-input" type="checkbox" id="flexSwitchCheckChecked" name="status" @if($radioNews->status == 1)
                                    checked
                                    @endif>
                ...
                <div class="ml-2 mb-2">
                    <a href="{{route('radio-news.index')}}" class="btn btn-secondary">Отмена</a>
