...
@section('content')
    ...
    <div class="card">
        <div class="d-flex align-items-center justify-content-between mb-2">
            <h5 class="card-header">Видеорепортажи</h5>
            ...
            <div class="d-flex align-items-center">
                ...
                <a href="{{ route('video-reportages.create') }}" type="button" class="btn btn-primary waves-effect waves-light ms-2">Добавить</a>
            </div>
        </div>

        <!-- Фильтры и сортировка -->
        <div class="card-body border-bottom">
            <form method="GET" action="{{ route('video-reportages.index') }}" class="row g-3">
                ...
                <div class="col-md-3 d-flex align-items-end">
                    <div class="btn-group w-100">
                        ...
                        <a href="{{ route('video-reportages.index') }}" class="btn btn-outline-secondary">Сбросить</a>
                    </div>
                </div>
                ...
                @if(isset($videos) && $videos->count() > 0)
                    @foreach($videos as $item)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('home.news.single', $item->slug) }}" target="_blank">
                ...
                @else
                    <tr>
                        <td colspan="8" class="text-center py-4">
                            <div class="text-muted">Видеорепортажи не найдены</div>
                            @if(request()->anyFilled(['search', 'status', 'author', 'category']))
                                <a href="{{ route('video-reportages.index') }}" class="btn btn-sm btn-outline-primary mt-2">
                                    Сбросить фильтры
                                </a>
                            @endif
                        </td>
